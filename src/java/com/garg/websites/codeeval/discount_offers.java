...
public class discount_offers
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		try {
			...
			while ((msg = br.readLine()) != null) {
				...
				System.out.println(maxScore(tokens[0].split(","), tokens[1].split(",")));
			}
		}
		finally {
			if (br != null) {
	...
	private static double maxScore(String[] customers, String[] products) 
	{
		// compute the cost matrix first.
		// cost[customers][products] with cost[i][j] containing
		// ss of i'th customer buying jth product
		double cost[][] = new double[customers.length][products.length];
		
		for (int c = 0; c < customers.length; c++) {
			for (int p = 0; p < products.length; p++) {
				// -1, since we want maximize.
				cost[c][p] = -1 * suitabilityScore(customers[c], products[p]);
			}
		}
		
		HungarianAlgorithm ha = new HungarianAlgorithm(cost);
		int[] matching = ha.execute();
		// customer i chooses product matching[i]
		// if matching[i] == -1, customer is not used.
		double total = 0;
	 ...
	 * the number of vowels (a, e, i, o, u, y) in the customer's name multiplied
	 * by 1.5. 
	 * 2. If the number of letters in the product's name is odd then the
	 * SS is the number of consonants in the customer's name. 
	 * 3. If the number
	 * of letters in the product's name shares any common factors (besides 1)
	 * with the number of letters in the customer's name then the SS is
	 * multiplied by 1.5.
	 * @param vowelCount 
	 */
	private static double suitabilityScore(String c, String p) 
	{
		int lcc = letterCount(c);
		int lcp = letterCount(p);
		int vcc = vowelCount(c);
		double ss = lcc - vcc; // number of consonants.
		if (lcp % 2 == 0) {
			ss = vcc * 1.5;
		}
		int gcd = gcd(lcc, lcp);
